...
const P={bg:'#F1F3F7',wh:'#FFFFFF',bo:'#E2E8F0',ac:'#C4857A',tx:'#0F172A',ts:'#64748B',tm:'#94A3B8',gn:'#10B981',bl:'#3B82F6',am:'#F59E0B',rd:'#EF4444',pu:'#8B5CF6',or:'#F97316',gy:'#94A3B8',vi:'#6366F1'};
...
// Zero-pads any YYYY-M-D style date to YYYY-MM-DD. Dates are compared as plain
// strings everywhere in this app, so an unpadded month/day (e.g. "2026-3-15")
// sorts AFTER "2026-08-01" alphabetically and silently leaks into the wrong
// week/month filters. Every date entering the app — on load, on restore, on
// manual entry — runs through this first.
const normD=s=>{
...
const newCl=()=>({id:gid(),company:'',contactName:'',contactTitle:'',feeFirst:'',feeSubsequent:'',paymentTerms:'30 days',contractDate:'',contractLink:'',roles:[],invoices:[],notes:'',createdAt:new Date().toISOString()});
...
const newIn=()=>({id:gid(),invoiceNumber:'',roleTitle:'',candidateName:'',amount:'',currency:'AED',dateIssued:'',dateDue:'',datePaid:'',status:'Draft',link:''});
...
const INP={width:'100%',padding:'9px 12px',borderRadius:8,border:`1px solid ${P.bo}`,background:'#fff',color:P.tx,fontSize:13,fontFamily:'inherit',outline:'none',boxSizing:'border-box'};
const INP_TA={width:'100%',padding:'9px 12px',borderRadius:8,border:`1px solid ${P.bo}`,background:'#fff',color:P.tx,fontSize:13,fontFamily:'inherit',outline:'none',boxSizing:'border-box',resize:'vertical'};
const TH_S={padding:'10px 14px',textAlign:'left',color:P.ts,fontWeight:600,fontSize:11,textTransform:'uppercase',letterSpacing:'0.5px',whiteSpace:'nowrap',background:'#F8FAFC'};
...
function ISel({value,opts,cm,onSave}){
  ...
  return <select autoFocus value={value||''} onChange={e=>{onSave(e.target.value);setEd(false);}} onBlur={()=>setEd(false)} style={{border:`1px solid ${P.ac}`,borderRadius:6,padding:'4px 8px',fontSize:12,outline:'none',background:'#fff'}}>{(opts||[]).map(o=><option key={o} value={o}>{o}</option>)}</select>;
}
function IDt({value,onSave,today}){
  const[ed,setEd]=useState(false);
  const t=today;const od=value&&value<t;const isT=value===t;
  if(!ed)return <span onClick={()=>setEd(true)} style={{cursor:'pointer',fontSize:12,color:od?P.rd:isT?P.or:value?P.tx:P.tm,fontWeight:(od||isT)?600:400,borderBottom:'1px dashed #CBD5E1'}}>{value||'—'}{od?' ⚠':isT?' 🔔':''}</span>;
...
function ITx({value,onSave,ph,ml}){
  ...
  const st={border:`1px solid ${P.ac}`,borderRadius:6,padding:'4px 8px',fontSize:12,fontFamily:'inherit',outline:'none',background:'#fff',width:'100%'};
  if(!ed)return <span onClick={()=>setEd(true)} title="Click to edit" style={{cursor:'pointer',color:value?'inherit':P.tm,borderBottom:'1px dashed #CBD5E1',fontSize:12}}>{value||ph||'—'}</span>;
...
function Mod({title,onX,wide,children}){return(
  <div onClick={e=>{if(e.target===e.currentTarget)onX();}} style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:16,backdropFilter:'blur(4px)'}}>
    <div style={{background:'#fff',borderRadius:16,padding:28,width:'100%',maxWidth:wide?720:560,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(0,0,0,0.18)'}}>
...
export default function App(){
  ...
  const[targets,setTargets]=useState({calls:20,floats:10,meetings:5,leads:5});
  ...
  const[tgEdit,setTgEdit]=useState({calls:20,floats:10,meetings:5,leads:5});
  ...
  useEffect(()=>{(async()=>{
    ...
    const tg=await load(K.tg,{calls:20,floats:10,meetings:5,leads:5});setTargets(tg);setTgEdit({...tg});
    const dt=await load(K.dt,'');setDateOverride(typeof dt==='string'?dt:'');
    setLoaded(true);
  })();},[]);

  const persist=(key,data)=>storage.set(key,JSON.stringify(data)).catch(()=>{});
  ...
  if(!loaded)return <div style={{background:P.bg,height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12}}><div style={{fontSize:20,fontWeight:800,color:P.ac,letterSpacing:'3px'}}>STRATIUM</div><div style={{color:P.tm,fontSize:12}}>LOADING...</div></div>;
  ...
  return(
    <div style={{display:'flex',flexDirection:mobile?'column':'row',height:'100vh',background:P.bg,fontFamily:'Inter,-apple-system,sans-serif',color:P.tx,fontSize:13}}>
      ...
      {!mobile&&<aside style={{width:195,background:P.wh,borderRight:`1px solid ${P.bo}`,display:'flex',flexDirection:'column',flexShrink:0,position:'sticky',top:0,height:'100vh',overflowY:'auto'}}>
        <div style={{padding:'22px 18px 16px',borderBottom:`1px solid ${P.bo}`}}><div style={{fontWeight:800,fontSize:16,letterSpacing:'2.5px',color:P.ac}}>STRATIUM</div><div style={{fontSize:10,color:P.tm,letterSpacing:'2px',marginTop:1}}>BD CRM</div></div>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        {mobile&&<div style={{background:P.wh,borderBottom:`1px solid ${P.bo}`,padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:10,flexShrink:0}}>
          <div><div style={{fontWeight:800,fontSize:14,letterSpacing:'2px',color:P.ac}}>STRATIUM</div><div style={{fontSize:9,color:P.tm,letterSpacing:'2px'}}>BD CRM</div></div>
        ...
        <div style={{flex:1,overflowY:'auto'}}>
          ...
          {tab==='dash'&&<div style={{padding:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div><div style={{fontSize:22,fontWeight:700}}>Dashboard</div><div style={{fontSize:13,color:P.ts,marginTop:2}}>{parseD(T).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</div></div>
          ...
          {tab==='contacts'&&<div style={{padding:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <div style={{fontSize:18,fontWeight:700}}>BD Pipeline</div>
            ...
            <div style={{fontSize:11,color:P.ts,marginBottom:8}}>{filtCo.length} of {contacts.length} contacts · 📋 to view full conversation history</div>
            <div style={{...CARD,overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                ...
                <tbody>
                  ...
                  {filtCo.map((c,i)=><tr key={c.id} style={{borderBottom:`1px solid ${P.bo}`,background:i%2===0?P.wh:'#FAFBFD'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(310px,1fr))',gap:14}}>
              {filtCa.map(c=>{const cf=floats.filter(f=>f.candidateId===c.id||f.candidateName===c.name);return(
                <div key={c.id} style={{...CARD,padding:18}}>
                  ...
                  {c.notes&&<div style={{fontSize:12,color:P.ts,padding:'8px 10px',background:'#F8FAFC',borderRadius:7,marginBottom:10,borderLeft:`3px solid ${P.bo}`}}>{c.notes}</div>}
          ...
          {tab==='floats'&&<div style={{padding:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <div style={{fontSize:18,fontWeight:700}}>Float Tracker</div>
            ...
            <div style={{...CARD,overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                ...
                <tbody>
                  ...
                  {filtFl.map((f,i)=><tr key={f.id} style={{borderBottom:`1px solid ${P.bo}`,background:i%2===0?P.wh:'#FAFBFD'}}>
          ...
          {tab==='activity'&&<div style={{padding:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <div style={{fontSize:18,fontWeight:700}}>BD Activity</div>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <div style={{fontSize:11,color:P.tm,background:'#F8FAFC',border:`1px solid ${P.bo}`,borderRadius:6,padding:'4px 10px'}}>Today: <strong style={{color:P.tx}}>{T}</strong></div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:10,marginBottom:16}}>
              {[{type:'Call',label:'Calls',target:targets.calls,col:P.bl},{type:'Meeting',label:'Meetings',target:targets.meetings,col:P.pu},{type:'Float Email',label:'Floats',target:targets.floats,col:P.or},{type:'New Lead',label:'New Leads',target:targets.leads,col:P.gn},{type:'Client Signed',label:'Clients Signed',target:0,col:P.ac}].map(item=>{
                ...
                return <div key={item.type} style={{...CARD,padding:14}}>
                  ...
                  {item.target>0&&<div><div style={{height:4,background:'#EAECF0',borderRadius:99,overflow:'hidden'}}><div style={{height:'100%',width:`${pct}%`,background:item.col,borderRadius:99}}/></div><div style={{fontSize:10,color:P.tm,marginTop:3}}>{pct}% of target</div></div>}
                </div>;
              })}
            </div>
            <div style={{...CARD,overflow:'hidden',marginBottom:16}}>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,minWidth:480}}>
                  <thead><tr style={{borderBottom:`1px solid ${P.bo}`,background:'#F8FAFC'}}>{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((day,i)=><th key={day} style={{padding:'10px 8px',textAlign:'center',color:AWD[i]===T?P.ac:P.ts,fontWeight:AWD[i]===T?700:500,borderRight:i<6?`1px solid ${P.bo}`:'none'}}><div>{day}</div><div style={{fontSize:10,color:AWD[i]===T?P.ac:P.tm,marginTop:1}}>{AWD[i].slice(5)}</div></th>)}</tr></thead>
          ...
          {tab==='meets'&&<div style={{padding:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <div style={{fontSize:18,fontWeight:700}}>Meetings & Calls Log</div>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <div style={{fontSize:11,color:P.tm,background:'#F8FAFC',border:`1px solid ${P.bo}`,borderRadius:6,padding:'4px 10px'}}>Today: <strong style={{color:P.tx}}>{T}</strong>{dateOverride&&<span style={{color:P.or}}> (override)</span>}</div>
                <button style={btp()} onClick={()=>setAcM(true)}>+ Log Entry</button>
              </div>
            </div>
            <div style={{display:'flex',gap:8,marginBottom:12,alignItems:'center',flexWrap:'wrap'}}>
              ...
              {meetRange==='custom'&&(
                <div style={{display:'flex',gap:6,alignItems:'center',background:P.wh,borderRadius:8,border:`1px solid ${P.bo}`,padding:'6px 10px',flexWrap:'wrap'}}>
                  ...
                  <input type="date" value={meetFrom} onChange={e=>setMeetFrom(e.target.value)} style={{border:`1px solid ${P.bo}`,borderRadius:6,padding:'4px 8px',fontSize:12,outline:'none',background:'#fff',color:P.tx,fontFamily:'inherit'}}/>
                  <span style={{fontSize:11,color:P.ts,fontWeight:500}}>To</span>
                  <input type="date" value={meetTo} onChange={e=>setMeetTo(e.target.value)} style={{border:`1px solid ${P.bo}`,borderRadius:6,padding:'4px 8px',fontSize:12,outline:'none',background:'#fff',color:P.tx,fontFamily:'inherit'}}/>
                </div>
              )}
          ...
          {tab==='clients'&&<div style={{padding:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <div style={{fontSize:18,fontWeight:700}}>Clients</div>
              ...
              {clients.map(cl=>{
                ...
                return <div key={cl.id} style={{...CARD,padding:22}}>
                  ...
                  <div style={{display:'flex',marginBottom:cl.contractLink?10:14,borderRadius:8,border:`1px solid ${P.bo}`,overflow:'hidden'}}>
                    {[{l:'1st Role',v:`${cl.feeFirst||'—'}%`,c:P.gn},{l:'Subsequent',v:`${cl.feeSubsequent||'—'}%`,c:P.ac},{l:'Terms',v:cl.paymentTerms||'—',c:P.ts},{l:'Fees',v:fees?`AED ${(fees/1000).toFixed(0)}k`:'—',c:P.ac}].map((item,idx)=><div key={item.l} style={{flex:1,padding:'10px 6px',textAlign:'center',background:'#F8FAFC',borderRight:idx<3?`1px solid ${P.bo}`:'none'}}><div style={{fontWeight:700,fontSize:14,color:item.c}}>{item.v}</div><div style={{fontSize:10,color:P.tm,marginTop:2}}>{item.l}</div></div>)}
                  </div>
                  {cl.contractLink&&<div style={{marginBottom:12,padding:'7px 10px',background:'#F8FAFC',borderRadius:7,border:`1px solid ${P.bo}`,display:'flex',gap:8,alignItems:'center'}}><span style={{fontSize:11,color:P.ts}}>📄</span><a href={cl.contractLink} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:P.bl}}>View Contract</a></div>}
                  ...
                  {(cl.roles||[]).map(r=><div key={r.id} style={{padding:'9px 12px',background:'#F8FAFC',borderRadius:8,marginBottom:6,border:`1px solid ${r.status==='Placed'?`${P.gn}40`:P.bo}`}}>
                  ...
                  {(cl.invoices||[]).length>0&&<div style={{marginTop:10}}>
                    ...
                    {(cl.invoices||[]).map(inv=><div key={inv.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 10px',background:'#F8FAFC',borderRadius:7,marginBottom:4,border:`1px solid ${['Written Off','Waived'].includes(inv.status)?`${P.rd}30`:P.bo}`}}>
                      ...
                      <div style={{display:'flex',gap:6,alignItems:'center'}}>{inv.amount&&<span style={{fontSize:12,fontWeight:700,color:P.ac}}>AED {Number(inv.amount).toLocaleString()}</span>}<Bge l={inv.status} c={IC[inv.status]||P.ts}/>{inv.link&&<a href={inv.link} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:P.bl}}>🔗</a>}</div>
                    </div>)}
                  </div>}
                  {cl.notes&&<div style={{fontSize:12,color:P.ts,marginTop:10,padding:'8px 10px',background:'#F8FAFC',borderRadius:7,borderLeft:`3px solid ${P.bo}`}}>{cl.notes}</div>}
                </div>;
              })}
            </div>
          </div>}
      ...
      {clM&&<Mod title={clM==='add'?'Add Client':'Edit Client'} onX={()=>setClM(null)} wide={true}>
        ...
        {clF.roles.map(r=><div key={r.id} style={{padding:12,background:'#F8FAFC',borderRadius:8,marginBottom:8,border:`1px solid ${P.bo}`}}>
        ...
        {(clF.invoices||[]).map(inv=><div key={inv.id} style={{padding:12,background:'#F8FAFC',borderRadius:8,marginBottom:8,border:`1px solid ${P.bo}`}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'auto 1fr 1fr 1fr 1fr',gap:8,alignItems:'end'}}>
            ...
            <div><div style={LB_S}>Amount</div><input type="number" min="0" value={inv.amount||''} onChange={e=>updInv(inv.id,'amount',e.target.value)} style={INP}/></div>
      ...
      {repM&&<Mod title="Reports & Data" onX={()=>setRepM(false)}>
        <div style={{display:'grid',gap:10}}>
          <div style={{padding:'10px 12px',background:'#F8FAFC',borderRadius:8,border:`1px solid ${P.bo}`}}>
      ...
      {expD&&<Mod title="Export Data" onX={()=>setExpD(null)}>
        <div style={{fontSize:12,color:P.ts,marginBottom:8}}>Copy all → save as <code style={{background:'#F1F3F7',padding:'2px 6px',borderRadius:4,fontSize:11}}>.json</code></div>
        <textarea readOnly value={expD} style={{width:'100%',height:200,fontSize:10,fontFamily:'monospace',padding:10,borderRadius:8,border:`1px solid ${P.bo}`,background:'#F8FAFC',resize:'none',boxSizing:'border-box'}}/>
      ...
      {conf&&<div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000,padding:16}}>
        <div style={{background:'#fff',borderRadius:14,padding:24,maxWidth:400,width:'100%',boxShadow:'0 24px 64px rgba(0,0,0,0.2)'}}>
      ...
      {toast&&<div style={{position:'fixed',bottom:24,right:24,background:toast.t==='err'?P.rd:P.tx,color:'#fff',padding:'11px 20px',borderRadius:10,fontWeight:600,fontSize:12,zIndex:3000,boxShadow:'0 8px 24px rgba(0,0,0,0.2)',fontFamily:'inherit'}}>{toast.msg}</div>}
    </div>
  );
}
